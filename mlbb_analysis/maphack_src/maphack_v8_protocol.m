/*
 * MLBB Map Hack v8 - With WraithProtocol
 * 
 * STOLEN FROM WRAITH:
 * - WraithProtocol blocks TikTok anti-cheat endpoints
 * - Without it game crashes (anti-cheat kills the app)
 * - Must register in constructor BEFORE any other code
 */
...
// WraithProtocol - blocks anti-cheat endpoints
@interface WraithProtocol : NSURLProtocol
@end

@implementation WraithProtocol

+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    // Only handle requests once (prevent loop)
    if ([NSURLProtocol propertyForKey:@"WraithDone" inRequest:request]) {
        return NO;
    }
    return YES;
}
...
- (void)startLoading {
    ...
    [NSURLProtocol setProperty:@YES forKey:@"WraithDone" inRequest:newRequest];
    
    NSString *path = newRequest.URL.path;
    
    // TikTok/Douyin anti-cheat endpoints (from Wraith research)
...
// CRITICAL: Register WraithProtocol FIRST (blocks anti-cheat)
__attribute__((constructor))
static void maphack_init(void) {
    @autoreleasepool {
        // Register protocol to block anti-cheat (MUST be first!)
        [NSURLProtocol registerClass:[WraithProtocol class]];
        
        // Start map hack worker thread
