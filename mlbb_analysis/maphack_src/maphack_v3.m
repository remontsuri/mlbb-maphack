 ...
 * Minimal constructor, delayed init like Wraith
 */
...
static void* workerThread(void* arg) {
    ...
    for (int attempt = 0; attempt < 5 && g_running; attempt++) {
        findOffsets();
        if (bm_static_data && off_m_ShowPlayers > 0) break;
        sleep(5);
    }
    
    // Main loop - write to memory every 500ms
    while (g_running) {
...
static void maphack_init(void) {
    // Minimal constructor - just start background thread
